const timer = () => {
  ...
  const numWord = (value, words) => {
    value = Math.abs(value) % 100; //избавляемся от отрицательных значений
  ...
  const updateTimer = () => {
    const date = new Date();
    const dateDeadline = new Date("25 november 2026").getTime();
    ...
    if (timeRemaining <= 0) {
      ...
      daysBlock.nextElementSibling.textContent = numWord(0, [
        "день",
        "дня",
        "дней",
      ]);
      hoursBlock.nextElementSibling.textContent = numWord(0, [
        "час",
        "часа",
        "часов",
      ]);
      minutesBlock.nextElementSibling.textContent = numWord(0, [
        "минута",
        "минуты",
        "минут",
      ]);
      secondsBlock.nextElementSibling.textContent = numWord(0, [
        "секунда",
        "секунды",
        "секунд",
      ]);
  ...
  updateTimer();
  interval = setInterval(updateTimer, 500); //1000 мс вызывает неестественную задержку, лучше обновлять чаще
};
timer();
